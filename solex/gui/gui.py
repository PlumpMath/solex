...
class Pre_VOIUDFGHLJIKOGDFSKLiew_Win(_Window_):
    
    def update(self, sys):
        self.System_Banner.set_text(sys.name)
    def add_star_label(self, star):
        self.__add_Star_Label(star)
    def fart(self):
        pass
    ...
    class System_Banner(gui.Text):
        text = ""
        font = {'name':"cour.ttf",'size':24,'colour':FONT_COL}
        bg = (0,0,0,.5)
        pad = (10, 12)
    ...
    def __add_Star_Label(self, star):
        class Star_Label(gui.Button, gui._Obj_Tracker_):
            ...
            track_obj = star.preview_model
            def __init__(self, master, **attrs):
                gui.Button.__init__(self, master, **attrs)
                self._prev_pos = self.track_obj.getPos(self.CTRL.CAMERA.NP)
